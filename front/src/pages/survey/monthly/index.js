...
const MonthlySurvey = () => {
  ...
  const [additionalSurveyQuestions, setAdditionalSurveyQuestions] = useState(
    [],
  );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // メインサーベイの回答を送信
      await InsertMonthlySurveyAnswers();
  ...
  // 入力情報をDBに登録する。
  const InsertMonthlySurveyAnswers = async () => {
    ...
    try {
      const response = await axios.post('/api/survey_monthly_answers', {
        employee_id,
        survey_term_id,
  ...
  useEffect(() => {
    if (question && question.data) {
      setMonthlySurveyQuestions(question.data.slice(0, 2));
      setAdditionalSurveyQuestions(question.data.slice(3, 8));
    }
  }, [question]);
      ...
      >
        ...
        >
          ...
          >
            ...
            <h2>Q.パルスサーベイとは</h2>
            <p className="text-xs">
              A.調査のリアルタイム性を向上させるために、月に一度社員の皆様に回答していただくアンケートです。
            </p>
            <br />
            <h2>実施時間は？</h2>
            <p>A.約3分を想定しております。</p>
        ...
        >
          <form onSubmit={handleSubmit}>
            ...
            additionalSurveyQuestions.length > 0 ? (
              <Box>
                <Box
                  sx={{
                    textAlign: 'left',
                ...
                >
                  {additionalSurveyQuestions.map((item, index) => (
                    <AdditionalSurveyQuestionBox
                      key={item.survey_question_id}
                      questionNumber={index + 1}
                      question={item.question}
...
const SizeRadioButtons = ({ handleChange }) => {
